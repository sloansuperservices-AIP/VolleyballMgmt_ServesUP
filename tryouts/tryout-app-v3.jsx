...
function MetricInputCell({ athlete, station, idx, updateMetric, tdS }) {
  const vs = (athlete.metrics && athlete.metrics[station.key]) || [];
  return (
    <td key={station.key+idx} style={tdS}>
      <input type="number" value={vs[idx]||""} onClick={e=>e.stopPropagation()}
        onChange={e=>{
          const nv=[...(athlete.metrics&&athlete.metrics[station.key]||["","",""])];
          nv[idx]=e.target.value;
          updateMetric(athlete.id,station.key,nv);
        }}
              ...
              return (
                <tr key={a.id} style={{borderBottom:"1px solid #1e222b",cursor:"pointer"}} onClick={()=>onSelectPlayer(a)}
                  ...
                  {stationsToShow.map(station => station.metrics.map((m,idx) => (
                    <MetricInputCell key={station.key+idx} athlete={a} station={station} idx={idx} updateMetric={updateMetric} tdS={tdS} />
                  )))}
